...
def load_validation_examples(validation_path: str) -> List[Dict]:
    """Load and normalize validation examples; skip rows lacking id or text."""
    examples: List[Dict] = []
    bad_json = 0
    missing_id = 0
    missing_text = 0
    with codecs.open(validation_path, 'r', encoding='utf-8-sig', errors='replace') as f:
        for raw_line in f:
            line = raw_line.strip()
            if not line:
                continue
            ...
            examples.append(obj)
    print(
        f"Loaded validation: total={len(examples)}, bad_json={bad_json}, "
...
def build_predictions_full_pipeline_parallel(
    ...
    validation_path = os.path.join(PROJECT_ROOT, 'data', 'validation.jsonl')
    ...
    # Load and normalize full validation set
    all_examples = load_validation_examples(validation_path)
